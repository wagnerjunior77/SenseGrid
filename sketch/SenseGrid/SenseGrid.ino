...
// sketch/SenseGrid/SenseGrid.ino (VO=1s, MAX=200cm)
#include <Arduino.h>
...
static void radar_config_boot() {
  ...
  Serial.println(F("[CFG] VO hold = 1000 ms"));
  radar_send(SET_VO_HOLD_1S, sizeof(SET_VO_HOLD_1S));

  Serial.println(F("[CFG] Presence max = 200 cm"));
...
void setup() {
  ...
  Serial.println(F("\n[BOOT] SenseGrid tuned (VO=1s, MAX=200cm)"));
...
void loop() {
  ...
  if (millis() >= g_after_cfg_ms) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      // JSON de diagnóstico em uma linha (mantém compatibilidade com stream)
      Serial.print('{');
      Serial.print(F("\"ts_ms\":"));    Serial.print(g_last_seen_ms); Serial.print(',');
